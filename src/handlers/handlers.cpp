...
crow::response user_find_handler(const crow::request& req, Database& db) {
  ...
  auto query_result = db.get_collection<User>("users")
                          .filter({{"handle", request["handle"].s()}})
                          .apply();
...
crow::response message_send_handler(const crow::request& req, Database& db) {
  ...
  THROW_BAD_REQUEST_IF(!ValidateRequest(request, "sender", "reciever",
                                        "payload", "datetime", "encrypted_by"),
                       "Invalid JSON format");
  ...
  Message new_message{request["sender"].s(), request["reciever"].s(),
                      request["payload"].s(), request["encrypted_by"].s(), *tp};
...
crow::response message_get_handler(const crow::request& req, Database& db) {
  ...
  auto messages_query_sender =
      messages.filter(*from_tp, *to_tp)
          .filter({{"sender", request["encryptor"].s()},
  ...
  auto messages_query_reciever =
      messages.filter(*from_tp, *to_tp)
          .filter({{"sender", request["partner"].s()},
